...
<?php
    ...
    if (isset($_GET['del_id'])) { //проверяем, есть ли переменная на удаление
        $sql = mysql_query('DELETE FROM `history` WHERE `id` = '.$_GET['del_id']); //удаляем строку из таблицы
    }
     ...
     if (isset($_POST['sub'])=='OK') { //Проверяем, передана ли переменная на редактирования
        if (isset($_POST['name'])) { //Если новое имя предано, то обновляем и имя и цену
            $sql = mysql_query("UPDATE history SET name = '".$_POST[name]."',text = '".$_POST[text]."',text2 = '".$_POST[text2]."' WHERE id = ".$_POST[id].";");
...
<?php
$sql = mysql_query("SELECT `id`, `name`, `text`, `text2` FROM `history`", $link);
...
<?php
    if (isset($_GET['red_id'])) { //Если передана переменная на редактирование
        ...
        $sql = mysql_query("SELECT `id`, `name`, `text`, `text2` FROM `history` WHERE `id`=".$_GET['red_id'], $link); //запрос к БД
